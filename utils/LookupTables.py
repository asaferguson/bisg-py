...
class LookUpTables:
    def __init__(self,
                    ...
                    race_list=['white', 'black', 'api', 'aian', '2prace', 'hispanic'],
    ...
    def _update_tables(self, 
        ...
        self.pr_block_given_race, self.pr_tract_given_race, self.pr_zip_given_race=self._census_geoids(tract_data,        #MUST BE ON SAME LINE OTHERWISE BREAK
                                                                                                    zip_data,
                                                                                                    block_group_data,
                                                                                                    id_columns)
        pass

    #convert a census geo information to lookup tables
    ...
    def _census_geoids(self,tract_data:pd.DataFrame=None, zip_data:pd.DataFrame=None, block_group_data:pd.DataFrame=None,id_columns=['GEOID20_BlkGrp','GEOID20_Tract','ZCTA5']) -> list:
        ...
        if [1 if type(dft) == type(None)  else 0 for dft in list_of_datasets] ==[1, 1, 1]:
        ...
        for i in range(3):
            ...
            if file == 'zip':
                df = df.drop(df[df['ZCTA5'].str.startswith(('006','007','008','009'))].index)
            ...
            df['NH_API_alone'] = df['NH_API_alone']+df['NH_Asian_HPI']+df['NH_Asian_HPI_Other']
            ...
            assert list(df['Total_Pop']) == list(df[['NH_White_alone','NH_Black_alone','NH_API_alone','NH_AIAN_alone','NH_Mult_Total','NH_Other_alone','Hispanic_Total']].sum(axis=1).astype(int))
            ...
            for x in ['NH_White_alone','NH_Black_alone','NH_API_alone','NH_AIAN_alone','NH_Mult_Total']:
                df[x]= df[x] + (df[x]/(df["Total_Pop"] - df["Hispanic_Total"] - df["NH_Other_alone"])) * df["NH_Other_alone"]   

                #cover your bases in the div by 0 case
                df[x] = np.where(df["Total_Pop"] == 0, 0, df[x]) 
                df[x] = np.where(df["Non_Hispanic_Total"] == df["NH_Other_alone"], df["NH_Other_alone"]/5, df[x])
            ...
            df['pop_check'] = df[["NH_White_alone", "NH_Black_alone", "NH_AIAN_alone", "NH_API_alone", "NH_Mult_Total", "Hispanic_Total"]].sum(axis=1)
            ...
            assert list(df['Total_Pop'].astype(float).round(2))==      list( df['pop_check'].astype(float).round(2))
            ...
            GROUP3 = ["NH_White_alone", "NH_Black_alone", "NH_AIAN_alone", "NH_API_alone", "NH_Mult_Total", "Hispanic_Total", "Total_Pop"]
            ...
            GROUP4 = ["white","black", "aian", "api", "mult_other", "hispanic"]                         #These are the final races
            
            for x, y in zip(GROUP4,GROUP3[:6]):
                df["pr_" + x+"_given_geo"] = df[y].astype(float)/df["Total_Pop"].astype(float)
            ...
            GROUP5 = ["NH_White_alone", "NH_Black_alone", "NH_AIAN_alone", "NH_Mult_Total", "Hispanic_Total", "NH_API_alone","NH_Hawn_alone"]
            for x in GROUP5:
                #"Number of other-race or multiple-race non-Hispanics: `national_nh_mult_other'"
                pop_sum["national_NH_Mult_other"] = pop_sum["national_Total_Pop"] -pop_sum["national_" +x]
                
            #Dividing by national populations    
            # p(geoID | race) = n_{geoID and race}/n_race_total 
            #list(zip(GROUP4,GROUP3[:6]))
            for x, y in zip(GROUP4,GROUP3[:6]):
                df["pr_geo_given_" + x] = df[y].astype(float)/pop_sum["national_"+y]  #dividing vector by scalar here
            ...
            geo_col = [f"pr_{x}_given_geo" for x in self.race_list]#[col for col in df if col.startswith('geo_pr')]
            here_col =  [f"pr_geo_given_{x}" for x in self.race_list]#[col for col in df if col.startswith('here')]
    ...
    #this function is entirely specific to the census data format we currently have, and would basically need to be rebuilt for anything else
    def _census_surnames(self, surname_data:pd.DataFrame#, race_list=None #,surname_column=self.census_name_column
    ):
        census_race_list=['white', 'black', 'api', 'aian', '2prace', 'hispanic']
        ...
        if type(surname_data)!=list and surname_data != tuple:
            ...
            surname_data=[input_to_dataframe(dataframe) for dataframe in surname_data]
            ...
            for lower_priority_dataframe in surname_data[1:]:
                full_surname_dataframe=pd.concat([full_surname_dataframe,lower_priority_dataframe[~lower_priority_dataframe["name"].isin( full_surname_dataframe["name"])]])

        #very specific to our given data
        assert pd.DataFrame({"Nessary_Columns":["name","count"]+["pct"+race for race in census_race_list]}).isin(list(full_surname_dataframe.columns)).all().iloc[0], "Surnames Data Passed Does not Contain All the Necessary Columns"
        ...
        for x in ["pct"+race for race in census_race_list]:
            full_surname_dataframe[x]= pd.to_numeric(full_surname_dataframe[x], errors= 'coerce')
        ...
        full_surname_dataframe["countmiss"] = full_surname_dataframe[race_list_pct].isna().sum(axis=1)
        
        #calculate a field that sums the percentages of all races for each name
        full_surname_dataframe["remaining"] = full_surname_dataframe[race_list_pct].sum(axis=1)
        ...
        full_surname_dataframe["remaining"] = full_surname_dataframe["count"] * (1- full_surname_dataframe["remaining"])
        ...
        for x in race_list_pct:
            full_surname_dataframe[x] = np.where(full_surname_dataframe[x].isna(),
                                                    (full_surname_dataframe["remaining"]/((full_surname_dataframe["countmiss"]) * full_surname_dataframe["count"])),
                                                    full_surname_dataframe[x])
        
        full_surname_dataframe.drop(columns=["remaining","countmiss"], inplace=True)
        ...
        full_surname_dataframe.rename(columns = {f'pct{race}': f'pr_{races_to_final[race]}_given_surname' for race in  census_race_list}, inplace = True)

        #print(full_surname_dataframe.columns)
    ...
    #Defunct functions that lead to pointless abstraction and make the pipeline hard to read
    ...
    # print(race_list)
        for x in race_list_pct:
            df[x] = np.where(df[x].isna(),(df["remaining"]/((df["countmiss"]) * df["count"])),df[x])
