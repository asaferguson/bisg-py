...
class RacePredictor:
    def __init__(self, 
        ...
        if lookuptables is None:
            raise Exception("Race Predictor expected to be passed a LookupTables object")
    ...
    def _add_names_to_input_data(self, input_data=None,name_column:str=None, surname_table=None):
        ...
        name_set=set(self.LookUpTables.pr_race_given_surname['name'].unique())
        input_data[name_column]=input_data[name_column].apply(clean_name)
        input_data[name_column]=input_data[name_column].apply(lambda x: best_subname(x, name_set))
        ...
        full_table=  pd.merge(input_data,
                         surname_table[['name']+[f'pr_{race}_given_surname'for race in self.race_list]], 
    ...
    #does only one of the iterations of the for loop in geo_name_merger
    ...
    def _typed_predict(self,
        ...
        if lookup_table is None or lookup_type is None:
            raise Exception("Lookup Table and Lookup Type must both be specified")
        ...
        #dataframe to store internal columns. These will be saved if in debugging mode
        #internal_df=pd.DataFrame()
        
        
        #debugging prints stuff
        #input_data_with_names[GeoInd[lookup_type]]=input_data_with_names[GeoInd[lookup_type]]#.astype(str)
        #print(input_data_with_names[GeoInd[lookup_type]])
        #print(lookup_table["GeoInd"])
        #print(lookup_table)
        #print(type(input_data_with_names[GeoInd[lookup_type]]))
        #print(type(lookup_table["GeoInd"]))

        race_geo_merged_df = pd.merge(input_data_with_names, lookup_table, left_on = geo_column_name, right_on ="GeoInd", how = "left",indicator=True)
        ...
        for x in final_race_list:
            race_geo_merged_df["u_" + x] =  race_geo_merged_df[f'pr_{x}_given_surname'] * race_geo_merged_df["pr_geo_given_" + x]
        dummy_cols_list=["u_" + x for x in final_race_list]
        
        
        race_geo_merged_df['u_sum']=race_geo_merged_df[dummy_cols_list].sum(axis=1)
        for x in final_race_list :
            race_geo_merged_df["pr_" + x] = race_geo_merged_df["u_" + x]/ race_geo_merged_df['u_sum']
        
        #add the `here columns`` to the internal_df for debugging
        #here_columns=['pr_geo']+[f'pr_geo_given_{race}' for race in final_race_list]
        #if self.debug_mode:
        #    internal_df[here_columns]=race_geo_merged_df[here_columns]

        #drop columns with "here" from final df
        #race_geo_merged_df.drop(columns = here_columns, inplace = True)
        
        
        ## Begin a series of checks 
        pr_race_list =  ['pr_'+race for race in final_race_list]
        race_geo_merged_df['prtotal'] = race_geo_merged_df[pr_race_list].sum(axis=1)#apply(lambda row: sum(row), axis=1)
        ...
        for x in final_race_list: 
             race_geo_merged_df['pr_'+x] = np.where(race_geo_merged_df['prtotal'] < .99, pd.NA, race_geo_merged_df['pr_'+x])
        ...
        for x in final_race_list:    
            assert ((race_geo_merged_df['pr_'+x].isnull()) | ((race_geo_merged_df['pr_'+x] >= 0) & (race_geo_merged_df['pr_'+x] <= 1))).all()
           
            
        #Assert that the total probabilities are sensible (probs sum to 1 or zero (could be 0 if census out of date?)) 
        race_geo_merged_df['check_name'] =  race_geo_merged_df[[f"pr_{race}_given_surname" for race in final_race_list]].sum(axis=1) 
        race_geo_merged_df['check_geo'] =   race_geo_merged_df[[f"pr_{race}_given_geo" for race in final_race_list]].sum(axis=1)
        race_geo_merged_df['check_pr'] =    race_geo_merged_df[["pr_"+race for race in final_race_list]].sum(axis=1)
        

        for y in ['name', 'geo', 'pr']: 
                assert ((race_geo_merged_df['check_'+y] == 0.0) |( (race_geo_merged_df['check_'+y] >= .99) & (race_geo_merged_df['check_'+y] <=1.01) )|(race_geo_merged_df['check_'+y].isna()) ).all(), f'Not all {y} probabilities sum to one. Throwing an error.'
        ...
        #rename final probability columns 
        #full_probability_dictionary={f'pr_{race}': f'{lookup_type}_pr_{race}_given_geo_and_surname' for race in final_race_list}
        #geo_probability_dictionary={f'pr_{race}_given_geo': f'{lookup_type}_pr_{race}_given_geo' for race in final_race_list}
        full_probability_dictionary={f'pr_{race}': f'pr_{race}_given_geo_and_surname' for race in final_race_list}
        geo_probability_dictionary={f'pr_{race}_given_geo': f'pr_{race}_given_geo' for race in final_race_list}
        #race_geo_merged_df.rename(columns = geo_probability_dictionary, inplace = True)
        race_geo_merged_df.rename(columns = full_probability_dictionary, inplace = True)   
        
        surname_columns=[f'pr_{race}_given_surname'for race in final_race_list]
        ...
        # remove all intermediate probaility columns if requested
        if only_final_probs:
            return race_geo_merged_df[[row_id_column,'GeoInd']+full_probability_columns]
        probability_and_id_columns=[row_id_column,'GeoInd']+full_probability_columns+geo_columns+surname_columns
    ...
    def predict(self, 
        ...
        #warnings
        ################## go here. Raise warning if predicting on geoid but not zip
        if zip_column==None and GEOID_column is not None:
            warnings.warn('Predicting at tract level without predicting at zip level. Please make sure this is intentional behavior.')
        
        if name_column is None:
            raise Exception("Name column of input data must be passed to prediction method.")
        
        if type(input_data)==str:
            if input_data[-3:]=='dta':
                warnings.warn('.dta data type supplied for input data. Make certain that GEOID and ZCTA5 are read in as str type, otherwise merge will fail.')

        

        #enforce types on parameters
        if type(name_column)!=str:
            raise TypeError("name_column must be of type str")

        if type(only_final_probs)!=bool:
            raise TypeError("only_final_probs should be boolean value")

        if type(blk_grp)!=bool:
            raise TypeError("blk_grp should be boolean value")
        if blk_grp and GEOID_column is None:
            raise Exception("blk_grp can only be True if GEOID_column is provided")
        
        if type(row_id_column) != str and (row_id_column is not None):
        ...
        if row_id_column is None:
            ...
            warnings.warn("No row_id_column passed. Creating a column 'rownum' for unique row id")
        input_data=self._add_row_id_column(input_data, row_id_column=row_id_column)
        ...
        if GEOID_column is not None:
            assert input_data_with_names[GEOID_column[0]].apply(lambda x: len(x)>=11 if type(x)==str else pd.NA).all(), "Not all GEOIDs of length atleast eleven, but GEOID_column argument given"  #this could be disabled
            input_data_with_names['tract']=input_data_with_names[GEOID_column[0]].apply(lambda x: x[:11] if type(x)==str else pd.NA)
            
        if blk_grp:
            #check to see that the geoid is greater than or equal to 12 or na
            assert input_data_with_names[GEOID_column[0]].apply(lambda x: len(x)>=12 if type(x)==str else pd.NA).all(), "Not all GEOIDs of length atleast twelve, but blk_grp = True"               #this could be disabled
            input_data_with_names['blk_grp']=input_data_with_names[GEOID_column[0]].apply(lambda x: x[:12] if type(x)==str else pd.NA)
        ...
        # find the name of the column in the input data which contains the relevant geo tag
...
#######################
        ...
        block_group=self._typed_predict(input_data_with_names=input_data_with_names,lookup_table=self.LookUpTables.pr_block_given_race, lookup_type="blkgrp", GeoIndNames=GeoIndNames)
        tract_group=self._typed_predict(input_data_with_names=input_data_with_names,lookup_table=self.LookUpTables.pr_tract_given_race, lookup_type="tract", GeoIndNames=GeoIndNames)
        zip_group=self._typed_predict(input_data_with_names=input_data_with_names,lookup_table=self.LookUpTables.pr_zip_given_race, lookup_type="zip", GeoIndNames=GeoIndNames)

        #Merge Block and TRact files together on rownum 
        merged_df1 = pd.merge(block_group, tract_group, on = "rownum", how = "left")
        ...
        bisg = pd.merge(merged_df2, input_data_with_names[[row_id_column, 'geo_code_precision']], on = "rownum", how = "left")
        ...
        for g in ['blkgrp18', 'tract18', 'zip18']:
            bisg['sum_'+g+'_pr'] = bisg[[x for x in bisg.columns if g+'_pr_' in x]].sum(axis=1)
            
        
        bisg['pr_precision'] = np.nan
        bisg['pr_precision'] = np.where((bisg['sum_zip18_pr'].notna()) & (bisg['sum_zip18_pr'] > 0) & (bisg['pr_precision'].isna()) & (bisg['geo_code_precision'].isin(['USAStreetName','USAZIP4','USAZipcode'])), 2,  bisg['pr_precision'])
        bisg['pr_precision'] = np.where((bisg['sum_blkgrp18_pr'].notna()) & (bisg['sum_blkgrp18_pr'] > 0) & (bisg['pr_precision'].isna()) & (bisg['geo_code_precision'].isin(['USAStreetAddr'])), 3,  bisg['pr_precision'])
        bisg['pr_precision'] = np.where((bisg['sum_tract18_pr'].notna()) & (bisg['sum_tract18_pr'] > 0) & (bisg['pr_precision'].isna()) & (bisg['geo_code_precision'].isin(['USAStreetAddr'])), 4,  bisg['pr_precision'])
        bisg['pr_precision'] = np.where((bisg['sum_zip18_pr'].notna()) & (bisg['sum_zip18_pr'] > 0) & (bisg['pr_precision'].isna()) & (bisg['geo_code_precision'].isin(['USAStreetAddr'])), 5,  bisg['pr_precision'])
        bisg['pr_precision'] = np.where(bisg['pr_precision'].isna(), 1,  bisg['pr_precision'])
        ...
        prob_labels = {1: 'NO FINAL PROB ASSIGNED', 2: 'ZIP (not rooftop lat/long)', 3: 'BLKGRP (has rooftop lat/long)', 4: 'TRACT (has rooftop lat/long)', 5: 'ZIP (has rooftop lat/long'}
        bisg['prob_lables'] = bisg['pr_precision'].map(prob_labels)
        
        #display frequencies 
        freq_table = bisg['pr_precision'].value_counts(dropna = False)
        print(freq_table)
        freq_table2 = pd.crosstab(bisg['pr_precision'], bisg['geo_code_precision'], margins=True)
        print(freq_table2)
        
        #choose what probability to use based on geography
        for r in ['white', 'black', 'hispanic', 'api', 'aian', 'mult_other']:
            bisg['pr_'+r] = np.nan
            bisg['pr_'+r] = np.where((bisg['pr_precision']== 2), bisg['zip18_pr_'+r] , bisg['pr_'+r])
            bisg['pr_'+r] = np.where((bisg['pr_precision']== 3), bisg['blkgrp18_pr_'+r] , bisg['pr_'+r])
            bisg['pr_'+r] = np.where((bisg['pr_precision']== 4), bisg['tract18_pr_'+r] , bisg['pr_'+r])
            bisg['pr_'+r] = np.where((bisg['pr_precision']== 5), bisg['zip18_pr_'+r] , bisg['pr_'+r])
            
        #check that final probabilities sum to one
        bisg['check_pr'] = bisg[['pr_white', 'pr_black', 'pr_hispanic', 'pr_api', 'pr_aian', 'pr_mult_other']].sum(axis=1)
        ...
        final_bisg = bisg[[row_id_column, 'geo_code_precision']+[col for col in bisg if col.startswith(('pr','blkgrp18_pr_','tract18_pr_','zip18_pr_','blkgrp18_geo_pr_','tract18_geo_pr_','zip18_geo_pr_','name_pr'))]]
        #print(final_bisg)
        #Export to CSV 
